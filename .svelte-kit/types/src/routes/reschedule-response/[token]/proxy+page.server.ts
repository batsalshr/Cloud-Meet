...
export const load = async ({ params, url, platform }: Parameters<PageServerLoad>[0]) => {
...
export const actions = {
	accept: async ({ params, platform }: import('./$types').RequestEvent) => {
		const db = platform?.env?.DB;
	...
	decline: async ({ params, platform }: import('./$types').RequestEvent) => {
		const db = platform?.env?.DB;
		const env = platform?.env;
		if (!db || !env) {
			return fail(500, { error: 'Database not available' });
